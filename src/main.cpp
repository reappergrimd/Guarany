...
// Copyright (c) 2014 The Guarany developers
...
#if defined(NDEBUG)
# error "Guarany cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6568e055107087b2fed2bff74292f3e42616562f3135f1d8cca76ebf90e3aea8");
uint256 hashGenesisBlockTestNet("0xf817b660b2518a9204a528dd764d0d1688224531f3794620246fe259f9008bf3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Guarany: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Guarany Signed Message:\n";
...
{
    // Guarany: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 0) {
        ...
        // Premine: 1% das GUARANYS são para swap
...
const int64 nTargetSpacing = 2.5 * 60; // Guarany: 2.5 minutos por bloco
static const int64 nTargetTimespan = 2 * 60; // Guarany: Not used
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    static const int64 BlocksTargetSpacing = 2.5 * 60; // 1 Minute
    ...
    if (pindexLast->nHeight < 6000)
    {
        ...
        PastSecondsMax = TimeDaySeconds * 0.7;
    }
...
{
    // Guarany: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Criando novo Genesis, dias de glória virão.";
        CTransaction txNew;
        txNew.nVersion = 1;
        txNew.nTime = 1415971319;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1415971319;
        block.nBits = 0x1e0ffff0;
        block.nNonce = 631819;

        if (fTestNet)
        {
            txNew.nTime = block.nTime = 1415971983;
            block.nNonce = 2798484;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x20ce22b97beb292c1032c566d58108b24a7354b3105406c9fb256ec47552ed0e"));
		block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Guarany: increase each by adding 2 to bitcoin's value.
