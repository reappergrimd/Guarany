...
template<typename Stream>
uint64 ReaGuaranypactSize(Stream& is)
{
    ...
    if (nSizeRet > (uint64)MAX_SIZE)
        THROW_WITH_STACKTRACE(std::ios_base::failure("ReaGuaranypactSize() : size too large"));
    return nSizeRet;
}
...
{
    unsigned int nSize = ReaGuaranypactSize(is);
    str.resize(nSize);
    if (nSize != 0)
