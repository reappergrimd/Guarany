...
void StakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("guarany-stakeminer");
...
void static GuaranyMiner(CWallet *pwallet)
{
    printf("GuaranyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("guarany-miner");
    ...
    try { loop {
        ...
        {
            printf("GuaranyMiner : no more PoW blocks\n");
            return;
        }

        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        printf("Running GuaranyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GuaranyMiner terminated\n");
        throw;
    }
}

void GenerateGuaranys(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GuaranyMiner, pwallet));
}
